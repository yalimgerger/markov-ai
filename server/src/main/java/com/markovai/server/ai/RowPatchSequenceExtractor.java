...
public class RowPatchSequenceExtractor implements MultiSequenceExtractor {
    ...
    public List<int[]> extractSequences(int[][] binaryImage) {
        ...
        for (int r = 0; r < GRID_SIZE; r++) {
            int[] rowSeq = new int[GRID_SIZE];
            for (int c = 0; c < GRID_SIZE; c++) {
                rowSeq[c] = getPatchSymbol(binaryImage, r, c);
            }
            sequences.add(rowSeq);
        }
        return sequences;
    }

    // Helper to get symbol (same encoding as PatchSequenceExtractor)
